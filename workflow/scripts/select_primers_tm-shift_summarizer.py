...
allele_53_path =os.path.join(snakemake.params.basedir, f'ref_allele/{snakemake.wildcards.variant_id}_ref_1_53.for')
allele_35_path = os.path.join(snakemake.params.basedir, f'ref_allele/{snakemake.wildcards.variant_id}_ref_1_35.for')
...
for output, orientation in zip(allele_paths, ['53', '35']):
    for n, row in read_primer3_out(output).iterrows():
        ...
        primer = Allele_specific(snakemake.wildcards.variant_id, snakemake.params.basedir)
...
for file in glob.glob(snakemake.params.basedir + f"/ref_allele/{snakemake.wildcards.variant_id}*"):
    if file[-3:] in ['rev']:
        primer_type = file[-3:]
        name = file.split('/')[-1][:-4]
        params = name.split('_')
...
if len(list(options.tables.keys())) > 0:
    ...
    os.mkdir(snakemake.output.summary)
    merged_path = f'{snakemake.output.summary}/{snakemake.wildcards.variant_id}_combinations.csv'
    out_merged.to_csv(merged_path, index = False)

    stacked_path = f'{snakemake.output.summary}/{snakemake.wildcards.variant_id}_primers.csv'
